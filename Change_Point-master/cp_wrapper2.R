cp_wrapper2 <- function(Data, isDiscrete, test, Crit, ratname, index, graphFolder)
{
        # 
        ...
        #
        # 4th argument: rat's name (pick from object "rats")
        # 5th argument: learning index (pick from object "Lindex")
        ...
        #### Plotting   ####
        ####################
        titlelabel = paste(ratname, index[2], sep="_")
        subFolder = paste(graphFolder, index[1], sep="")
        filetitle = paste(subFolder, "/", paste(ratname, index[1]), ".pdf", sep="")
        ...
        if  (isDiscrete == 0)
        {
                title(xlab = 'Time', ylab = 'Number of Events', main=titlelabel)
        }else
        {
                title(xlab = 'Trials', ylab = 'Cumulative response measure', main=titlelabel)
        }
        ...
        if (isDiscrete == 0)
        {
                title(xlab = 'Time' , ylab = 'Average Rate = Events/(Unit Time)', main=titlelabel)
        }else
        {
                title(xlab = 'Trials', ylab = 'Average Response per Trial', main=titlelabel)
        }
        dev.off()
        
        #Calculate the slopes and lengths of intervals between change points
        odd <- function(v){oddIdx <- seq(1, length(v), by=2); v[oddIdx]}
        allslopes <- odd(slopes(CP)[,2])
        lengthInt <- c(diff(CP)[,1], 0)
        
        #Add this to the data frame to be returned
        ratvec <- rep(ratname, nrow(CP))
        CP2 <- data.frame(ratvec, CP[,1], CP[,2], c(allslopes[2:length(allslopes)], 0), lengthInt)
        colnames(CP2)<- c('rat', 'CP_trial', paste("cum_", index[1], sep=""), 'slope', 'lengthInterval')
